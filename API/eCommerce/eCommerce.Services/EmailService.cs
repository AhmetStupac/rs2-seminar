...
namespace eCommerce.Services
{
    public class EmailService : IEmailService
    {
        ...
        public async Task SendLoginNotificationEmailAsync(LoginNotificationMessage message)
        {
            var smtpHost = _configuration["EmailSettings:SmtpHost"];
            var smtpPort = int.Parse(_configuration["EmailSettings:SmtpPort"] ?? "587");
            var smtpUsername = _configuration["EmailSettings:SmtpUsername"];
            ...
            var mailMessage = new MailMessage
            {
                ...
                Body = GetLoginEmailBody(message.FirstName, message.LastName, message.LoginTime),
                IsBodyHtml = true
            };
            ...
            {
                throw new Exception($"Error sending login notification email: {ex.Message}");
            }
        }

        private string GetLoginEmailBody(string firstName, string lastName, string loginTime)
        {
            return $@"
<!DOCTYPE html>
<html>
